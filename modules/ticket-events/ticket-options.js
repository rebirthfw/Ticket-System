          ...
          switch (interaction.customId) {
            case "ticket-close":
              if (data.closed == true) return interaction.reply({
                embeds: [new MessageEmbed()
                  .setTitle(`Этот билет уже закрыт.`)
                  .setColor(ee.wrongcolor)
                ],
                ephemeral: true
              })
...
Обработал: **${TranscriptClimedByUser.user.tag}** (${TranscriptClimedByUser.id})`)
            ...
            case "ticket-delete":
              interaction.reply({
                embeds: [new MessageEmbed()
                  .setTitle(`Билет будет удалён через 10 секунд!`)
                  .setColor(ee.wrongcolor)
                ]
              })
        ...
        interaction.reply({
          embeds: [new MessageEmbed()
            .setDescription(`${client.allEmojis.x} **У Вас нет необходимых прав, чтобы прикасаться к этим кнопкам!**`)
            .setColor(ee.wrongcolor)
          ],
          ephemeral: true
        })
